...
class Game:
    ...
    def reset(self):
        self.env.reset();
        self.last_screen = get_screen()
        self.current_screen = get_screen()
    ...
    def step(self):
        ...
        _, reward, done, _ = self.env.step(action.item()) #node, and reward
        reward = torch.tensor([reward], device=device)
        ...
        # Store the transition in memory
        memory.push(self.state, action, self.next_state, self.reward)
